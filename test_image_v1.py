...
checkpoint = torch.load('train_files/best_model_lyl_80.pth', weights_only=True)
...
def get_emo(path):
    ...
    max_index = np.argmax(detect_emotion(Image.fromarray(image)))
    max_emotion = emotions[max_index]
    return max_emotion

path = 'data/test/fear/PrivateTest_134207.jpg'
emo = get_emo(path)
print(f"emo:{emo}")
